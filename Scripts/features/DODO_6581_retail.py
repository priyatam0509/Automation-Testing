...
class DODO_6581_retail():
    ...
    @test
    def Papercheck_Cashadvance_Yes_Retail(self):
        """
        In a transaction with a paper check at a retail dispenser, the prompt code is correctly reported to the host.
        """
    ...
    @test
    def PostPay_Cashadvance_Papercheck_Retail(self):
        """
        When a postpaid transaction is started outside and a paper check is used for payment, this operation is completed successfully. So you inform the host of your prompt code correctly.
        """
        ...
        self.log.info("Select Auth button")
        self.pos.click_forecourt_key("auth")

        self.helpers.fuel_handler(amounts_to_dispense)

        self.pos.wait_for_fuel(timeout=240)

        self.log.info("Select Buffer A")
        self.pos.click_fuel_buffer('A')
