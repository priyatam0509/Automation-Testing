"""
    File name: SL-1901.py
    Brand:  Concord
    Description: Technology Indicator for Wex
    Author: Paresh
...
class SL_1901():
    ...
    def TC01(self):
        """ 
        Description : To verify when EMV Wex card is swiped on pin pad magstripe reader then POS is used the magstripe reader to process the card when performed prepay & dry stock transactions
        Args: None
        Returns: None
        """
        ...
        pos.click("Card")
        try:
            pinpad.swipe_card(card_name= "WEX1_EMV")
            time.sleep(2)
        ...
        # Verify message if we are swipping the wex emv card
        if "Swipe not allowed on EMV cards. Please insert" not in message:
            tc_fail("Error message is not displayed for wex card")
        
        # Complete transaction using wex emv card
        pos.click("Cancel")
        pos.click_message_box_key("OK")

        pos.pay_card(card_name = "WEX1_EMV")
    ...
    def TC02(self):
        """ 
        Description : To verify when EMV Wex card is swiped on pin pad magstripe reader then POS is used the chip card reader to process the card when performed postpay & dry stock transactions
        Args: None
        Returns: None
        """
    ...
    def TC03(self):
        """ 
        Description : To verify when EMV Wex card is swiped on pin pad magstripe reader after EMV fallback then POS is used the magstripe reader to process the card when performed manual fuel sale & dry stock transactions 
        Args: None
        Returns: None
        """
